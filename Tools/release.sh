...
#
# By default, increments the final component of MARKETING_VERSION (e.g.
# 1.0.3 → 1.0.4, 1.0 → 1.1) and resets the build number to 1. Pass
# `--version X.Y.Z` to set the next marketing version explicitly. Pass
# `--build-only` to ship another build of the current marketing version,
# which bumps the build number by 1.
#
# Usage:
#   Tools/release.sh                            # bump marketing patch + reset build to 1, tag v-<new-version>
#   Tools/release.sh some-name                  # bump marketing patch + reset build to 1, tag v-some-name
...
#   Tools/release.sh --build-only               # bump build only, auto-name timestamp tag
...
if [ "$build_only" = true ] && [ -n "$new_marketing" ]; then
    echo "ERROR: --build-only cannot be combined with --version." >&2
    exit 1
fi
...
if [ -z "$name" ]; then
    if [ "$build_only" = true ]; then
        ...
        # Marketing-version releases default to v-<version> so they're
        # instantly recognizable (e.g. v-1.0.3) instead of a timestamp.
